# PlazaOS — Developer Setup

> This document is for developers setting up PlazaOS locally or deploying to production.
> For an overview of the product, see [README.md](README.md).
...
Your web server isn't pointing to the `public/` directory, or URL rewriting isn't configured. With Caddy, the `root` directive must end with `/public`. Check the Caddyfile.
