                    ...
                    <div class="p-6">
                        <form @submit.prevent="submit">
                            <div class="mb-4">
                                ...
                                <input v-model="editForm.title" type="text" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-400 focus:ring-indigo-400 sm:text-sm" />
                                <p v-if="editForm.errors.title" class="mt-1 text-xs text-red-600">{{ editForm.errors.title }}</p>
                            </div>

                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Description</label>
                                <textarea v-model="editForm.description" rows="3" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-400 focus:ring-indigo-400 sm:text-sm" />
                                <p v-if="editForm.errors.description" class="mt-1 text-xs text-red-600">{{ editForm.errors.description }}</p>
                            </div>

                            <div class="mb-4">
                                <label class="flex items-center gap-2 text-sm font-medium text-gray-700">
                                    <input v-model="editForm.is_active" type="checkbox" class="rounded border-gray-200 text-indigo-500 shadow-sm focus:ring-indigo-400" />
                                    Active
                                </label>
                            </div>
                                ...
                                <div v-for="(field, index) in editForm.fields" :key="field.id ?? `new-${index}`" class="rounded-lg border border-gray-200 p-4">
                                    <div class="mb-3 flex items-center justify-between">
                                        ...
                                        <div class="flex gap-1">
                                            ...
                                            <button type="button" @click="moveField(index, 'down')" :disabled="index === editForm.fields.length - 1" class="rounded px-2 py-1 text-xs text-gray-600 hover:bg-gray-100 disabled:opacity-30">&darr;</button>
                            ...
                            <div v-if="editForm.fields.length === 0" class="mb-4 rounded-lg border-2 border-dashed border-gray-200 p-8 text-center text-sm text-gray-600">
