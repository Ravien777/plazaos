                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <select v-model="status" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" @change="applyFilters">
                            ...
                            <select v-model="priority" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" @change="applyFilters">
                                <option value="">All Priorities</option>
                                <option value="low">Low</option>
                                <option value="medium">Medium</option>
                                <option value="high">High</option>
                            </select>
                            <select v-model="category" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" @change="applyFilters">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Subject</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Related To</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Status</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Priority</th>
                                        <th class="px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Category</th>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="ticket in tickets.data" :key="ticket.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            <Link v-if="ticket.ticketable && ticket.ticketable_id" :href="ticketableLink(ticket)" class="text-indigo-600 hover:text-indigo-900">
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ priorityLabel(ticket.priority) }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ categoryLabel(ticket.category) }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ ticket.created_at }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm">
                                            <Link :href="`/tickets/${ticket.id}/edit`" class="text-indigo-600 hover:text-indigo-900">Edit</Link>
                                            <button class="ml-2 text-red-600 hover:text-red-900" @click="destroy(ticket.id)">Delete</button>
                                        </td>
                                    </tr>
                                    <tr v-if="tickets.data.length === 0">
                                        <td colspan="7" class="px-3 py-8 text-center text-sm text-gray-500">No tickets found.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
