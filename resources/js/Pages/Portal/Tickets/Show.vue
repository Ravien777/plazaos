    ...
    <PortalLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-700">{{ ticket.subject }}</h2>
                <Link :href="route('portal.tickets.index')" class="text-sm text-indigo-500 hover:text-indigo-600">&larr; Back</Link>
            </div>
        </template>
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-xs text-gray-600">{{ ticket.priority }} priority &middot; {{ ticket.category }}</span>
                        </div>
                        <div v-if="ticket.description" class="mt-4 whitespace-pre-wrap text-sm text-gray-700">{{ ticket.description }}</div>
                        <p class="mt-3 text-xs text-gray-500">Created {{ ticket.created_at }}</p>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div v-if="ticket.replies && ticket.replies.length > 0" class="mt-4 space-y-4">
                            <div v-for="reply in ticket.replies" :key="reply.id" class="rounded-md border border-gray-200 p-4">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-800">{{ reply.user?.name ?? 'Support' }}</span>
                        ...
                        <form @submit.prevent="submitReply" class="mt-6">
                            <textarea v-model="form.body" rows="3" class="block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-400 focus:ring-indigo-400 sm:text-sm" placeholder="Write a reply..." required></textarea>
