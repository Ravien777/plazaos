...
<template>
    ...
    <PortalLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">My Projects</h2>
        </template>
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">
                                    <tr v-for="project in projects.data" :key="project.id" class="hover:bg-gray-50">
                                        <td class="whitespace-nowrap px-3 py-4 text-sm font-medium text-gray-900">
                                            <Link :href="`/portal/projects/${project.id}`" class="text-indigo-600 hover:text-indigo-900">{{ project.name }}</Link>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4">
                                            <StatusBadge :status="project.status">{{ statusLabel(project.status) }}</StatusBadge>
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ project.budget ? `$${project.budget}` : '-' }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ project.start_date ?? '-' }}</td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ project.due_date ?? '-' }}</td>
                                    </tr>
                                    <tr v-if="projects.data.length === 0">
                                        <td colspan="5" class="px-3 py-8 text-center text-sm text-gray-500">No projects found.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
