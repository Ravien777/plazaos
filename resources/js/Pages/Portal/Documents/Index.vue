...
<template>
    ...
    <PortalLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">My Documents</h2>
        </template>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div v-if="documents.length === 0" class="text-sm text-gray-500">No documents available.</div>
                        <div v-else class="space-y-3">
                            <div v-for="doc in documents" :key="doc.id" class="flex items-center justify-between rounded-md border border-gray-200 p-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ (doc.size / 1024).toFixed(1) }} KB &middot; {{ doc.created_at }}</p>
                                </div>
                                <Link
